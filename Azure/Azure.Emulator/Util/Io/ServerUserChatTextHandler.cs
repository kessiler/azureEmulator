...
namespace Azure.Util.IO
{
    ...
    /// </summary>
    internal class ServerUserChatTextHandler
    {
        /// <summary>
        ...
        /// <param name="k">The k.</param>
        /// <returns>System.Int32.</returns>
        internal static int GetFirstSiffer(double k) => (int)System.Math.Round(k % 1 * 100);
        ...
        /// <returns>System.String.</returns>
        internal static string FilterHtml(string str, bool allow = false) => allow && ServerExtraSettings.AdminCanUseHtml ? str : Regex.Replace(str, @"</?(?(?=b|i)notag|[a-zA-Z0-9]+)(?:\s[a-zA-Z0-9\-]+=?(?:(["",']?).*?\1?)?)*\s*/?>", string.Empty);
    }
}