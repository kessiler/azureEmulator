...
namespace Azure.HabboHotel.Catalogs
{
    ...
    internal class Catalog
    {
        ...
        {
            var pet = new Pet(404u, userId, 0u, name, (uint)type, Race, Color, 0, 100, 150, 0, Azure.GetUnixTimeStamp(), 0, 0, 0.0, false, 0, 0, -1, Rarity, DateTime.Now.AddHours(36.0), DateTime.Now.AddHours(48.0), null)
            {
                DbState = DatabaseUpdateState.NeedsUpdate
            };
            using (var queryReactor = Azure.GetDatabaseManager().GetQueryReactor())
            {
                queryReactor.SetQuery(string.Concat("INSERT INTO bots (user_id,name, ai_type) VALUES (", pet.OwnerId, ",@", pet.PetId, "name, 'pet')"));
            ...
            uint @group)
        {
...

            {
                ...
                if (catalogPage.Layout == "vip_buy" || catalogPage.Layout == "club_buy" || HabboClubItems.Contains(item))
                {
                    ...
                    if (session.GetHabbo().Diamonds < item.DiamondsCost) return;
                ...
                if (session.GetHabbo().Diamonds < item.DiamondsCost * totalPrice) return;
            ...
            foreach (var baseItem in item.Items.Keys)
            {
                ...
                switch (interactionType)
                {
                    ...
                    case Interaction.Trophy:
                        extraData = string.Concat(session.GetHabbo().UserName, Convert.ToChar(9), DateTime.Now.Day.ToString("00"), "-", DateTime.Now.Month.ToString("00"), "-", DateTime.Now.Year, Convert.ToChar(9), extraData);
                        break;
                    ...
                    default:
                        switch (interactionType)
                        {
                            ...
                            case Interaction.BadgeDisplay:
                                ...
                                extraData = string.Format("{0}|{1}|{2}-{3}-{4}", extraData, session.GetHabbo().UserName, DateTime.Now.Day.ToString("00"), DateTime.Now.Month.ToString("00"), DateTime.Now.Year);
                ...
                if (isGift)
                {
                    var itemBySprite = Azure.GetGame()
                        .GetItemManager()
                        .GetItemBySpriteId(giftSpriteId);
