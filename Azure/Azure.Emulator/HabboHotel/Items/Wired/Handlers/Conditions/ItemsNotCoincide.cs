...
namespace Azure.HabboHotel.Rooms.Wired.Handlers.Conditions
{
    internal class ItemsNotCoincide : IWiredItem
    {
        ...
        {
            get { return Interaction.ConditionItemsDontMatch; }
        }
        ...
        public bool Execute(params object[] stuff)
        {
            ...
            foreach (var current in Items)
            {
                ...
                if (useRot) if (current.Rot != int.Parse(originalData[1])) return true;

                if (useExtradata)
                {
                    if (current.ExtraData == string.Empty) current.ExtraData = "0";

                    if (current.ExtraData != (originalData[0] == string.Empty ? "0" : originalData[0])) return true;
                }

                if (!usePos) continue;

                var originalPos = originalData[2].Split(',');
                if ((current.X != int.Parse(originalPos[0])) && (current.Y != int.Parse(originalPos[1]))) return true;
            }
