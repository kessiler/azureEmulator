...
namespace Azure.HabboHotel.Rooms.Wired
{
    public static class WiredSaver
    {
        ...
        {
            if (item == null || !item.IsWired) return;

            var room = item.GetRoom();
            if (room == null) return;

            var wiredHandler = room.GetWiredHandler();
            if (wiredHandler == null) return;
            ...
            switch (item.GetBaseItem().InteractionType)
            {
                ...
                case Interaction.ActionMoveToDir:
                {
                    ...
                    wired7.OtherString = string.Format("{0};{1}", dir, rot);
                ...
                case Interaction.ActionPosReset:
                {
                    ...
                    var booleans = string.Format("{0},{1},{2}", actualExtraData, actualRot, actualPosition).ToLower();
                    ...
                    foreach (var aItem in items)
                    {
                        if (aItem.GetBaseItem().InteractionType == Interaction.Dice)
                        {
                            dataToSave += string.Format("0|0|0|0,0,0", aItem.Id, aItem.ExtraData, aItem.Rot, aItem.X,
                                aItem.Y, aItem.Z);
                            extraStringForWi += string.Format("{0},{1},{2},{3},{4}", aItem.Id,
                                (actualExtraData) ? aItem.ExtraData : "N", (actualRot) ? aItem.Rot.ToString() : "N",
                                (actualPosition) ? aItem.X.ToString() : "N", (actualPosition) ? aItem.Y.ToString() : "N");
                            return;
                        }
                        dataToSave += string.Format("{0}|{1}|{2}|{3},{4},{5}", aItem.Id, aItem.ExtraData, aItem.Rot,
                            aItem.X, aItem.Y, aItem.Z);
                        extraStringForWi += string.Format("{0},{1},{2},{3},{4}", aItem.Id,
                            (actualExtraData) ? aItem.ExtraData : "N", (actualRot) ? aItem.Rot.ToString() : "N",
                            (actualPosition) ? aItem.X.ToString() : "N", (actualPosition) ? aItem.Y.ToString() : "N");
                        if (aItem == items.Last()) continue;
                ...
                case Interaction.ConditionNegativeHowManyUsers:
                {
                    ...
                    var ei = string.Format("{0},{1}", minimum, maximum);
                ...
                case Interaction.ConditionDateRangeActive:
                {
                    ...
                    var endDate = request.GetInteger(); //timestamps
                    ...
                    if (startDate == 0)
                    {
                        ...
                        // user_wired_con_date_range
                        // Sir Jamal => 9-2-2015 tested and working lel <3
                ...
                case Interaction.ActionGiveScore:
                {
                    ...
                    var newExtraInfo = string.Format("{0},{1}", scoreToGive, maxTimesPerGame);
