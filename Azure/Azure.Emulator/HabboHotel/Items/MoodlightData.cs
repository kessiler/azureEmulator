...
namespace Azure.HabboHotel.Items
{
    ...
    internal class MoodlightData
    {
        ...
        internal MoodlightData(uint itemId)
        {
            ...
            {
                queryReactor.SetQuery(string.Format("SELECT enabled,current_preset,preset_one,preset_two,preset_three FROM items_moodlight WHERE item_id='{0}'", itemId));
        ...
        internal void UpdatePreset(int preset, string color, int intensity, bool bgOnly, bool hax = false)
        {
            ...
            using (var queryReactor = Azure.GetDatabaseManager().GetQueryReactor())
            {
                queryReactor.SetQuery(string.Format("UPDATE items_moodlight SET preset_{0}='{1},{2},{3}' WHERE item_id='{4}'", text, color, intensity, Azure.BoolToEnum(bgOnly), ItemId));
