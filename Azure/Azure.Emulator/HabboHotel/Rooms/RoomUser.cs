    ...
    public class RoomUser : IEquatable<RoomUser>
    {
        ...
        internal void Chat(GameClient session, string msg, bool shout, int count, int textColor = 0)
        {
            ...
            if (!(msg.StartsWith(":deleteblackword ") && session.GetHabbo().Rank > 4) && BlackWordsManager.Check(msg, BlackWordType.Hotel, out word))
            {
                ...
                {
                    session.SendWhisper("A mensagem contém a palavra: " + word.Word + " que não é permitida, você poderá ser banido!");
                    return;
                }
            }
            ...
            chatMsg.Init(shout ? LibraryParser.OutgoingRequest("ShoutMessageComposer") : LibraryParser.OutgoingRequest("ChatMessageComposer"));
        ...
        internal void Serialize(ServerMessage message, bool gotPublicRoom)
        {
            ...
            if (BotData.AiType == AIType.Pet)
                if (PetData.Type == 16u)
                    ...
                    message.AppendString(string.Concat(BotData.Look.ToLower(), " 3 4 10 0 2 ", PetData.PetHair, " ", PetData.HairDye, " 3 ", PetData.PetHair, " ", PetData.HairDye));
                else if (PetData.HaveSaddle == Convert.ToBoolean(1))
                    message.AppendString(string.Concat(BotData.Look.ToLower(), " 3 2 ", PetData.PetHair, " ", PetData.HairDye, " 3 ", PetData.PetHair, " ", PetData.HairDye, " 4 9 0"));
                else
                    message.AppendString(string.Concat(BotData.Look.ToLower(), " 2 2 ", PetData.PetHair, " ", PetData.HairDye, " 3 ", PetData.PetHair, " ", PetData.HairDye));
            else
