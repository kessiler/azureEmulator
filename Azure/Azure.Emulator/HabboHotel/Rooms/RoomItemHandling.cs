    ...
    internal class RoomItemHandling
    {
        ...
        internal List<RoomItem> RemoveAllFurniture(GameClient session)
        {
            ...
            foreach (var item in FloorItems.Values.ToArray())
            {
                ...
                if (item.IsBuilder)
                {
                    ...
                    {
                        queryReactor.RunFastQuery(string.Format("DELETE FROM items_rooms WHERE id='{0}'", item.Id));
                    }
                    continue;
                }
                items.Add(item);
        ...
        internal void RemoveItemsByOwner(ref List<RoomItem> roomItemList, ref GameClient session)
        {
            ...
            foreach (RoomItem item in roomItemList)
            {
                ...
                GameClient Client = Azure.GetGame().GetClientManager().GetClientByUserId(item.UserId);
        ...
        internal void LoadFurniture()
        {
            ...
            using (var queryReactor = Azure.GetDatabaseManager().GetQueryReactor())
            {
                queryReactor.RunFastQuery(
                    "SELECT items_rooms.* , COALESCE(items_groups.group_id, 0) AS group_id FROM items_rooms LEFT OUTER JOIN items_groups ON items_rooms.id = items_groups.id WHERE items_rooms.room_id = " + _room.RoomId + " LIMIT 5000");
                ...
                foreach (DataRow dataRow in table.Rows)
                    try
                    {
                        ...
                        {
                            var roomItem = new RoomItem(id, _room.RoomId, baseItemId, extraData, x, y, z, rot, _room, ownerId,
                ...
                {
                    if (current.IsWired) _room.GetWiredHandler().LoadWired(_room.GetWiredHandler().GenerateNewItem(current));
        ...
        internal void DeveloperSetFloorItem(GameClient session, RoomItem item)
        {
            ...
            var affectedTiles = Gamemap.GetAffectedTiles(item.GetBaseItem().Length, item.GetBaseItem().Width, item.X, item.Y, item.Rot);
